...
class ChunkManager : Noncopyable
{
  ...
  public:
    ChunkManager(std::map<std::string,VoxelChunkData*>* chunkPtr,
    std::vector<glm::vec4>& pos, 
    std::vector<glm::vec2>& uv,
    std::vector<glm::vec3>& normal
    ) : _perlinNoise("default"), _chunkDatas(chunkPtr), _objectPos(pos), _objectUV(uv), _objectNormal(normal){};
    ~ChunkManager(){};//delete data

    void update(glm::vec3 _camPos);
